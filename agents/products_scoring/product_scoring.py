...
class ProductScoringAgent:
    def __init__(self, products_data: pd.DataFrame):
        self.products_data = products_data
        # Ensure that numeric columns are in the correct format
        num_columns = [col for col in self.products_data.columns if col not in \
                       ["id", "product_name", "product_category"]]
        self.products_data[num_columns] = (self.products_data[num_columns]
                                        .replace(",", ".", regex=True)
                                        .apply(pd.to_numeric, errors="coerce"))
        self.product_score_dict = {}

    def score_product(self, product_id):
        """
        ...
        logistics_simplicity = self.compute_logistics_simplicity(product_id)
        logger.info(f"Logistics simplicity score for product_id {product_id} is {logistics_simplicity}")
        supplier_reliability = self.compute_supplier_reliability(product_id)
        logger.info(f"Supplier reliability score for product_id {product_id} is {supplier_reliability}")
        tiktok_virality = self.compute_tiktok_virality(product_id)
        ...
        self.product_score_dict["supplier_reliability"] = supplier_reliability
        self.product_score_dict["tiktok_virality"] = tiktok_virality
    ...
    def compute_demand_growth(self, product_id):
        ...
        order_growth = self.compute_order_growth(product_id)
        ...
        self.product_score_dict["demand_growth_components"] = {
    ...
    def compute_competition_score(self, product_id):
        """
    ...
    def compute_expected_margin(self, product_id):
        """
        ...
        platform_fees = self.products_data.loc[self.products_data["id"] == product_id, 
                                            "platform_fees"].values[0]
        advertising_cost = self.products_data.loc[self.products_data["id"] == product_id, 
                                            "advertising_cost"].values[0]
        ...
        self.product_score_dict["expected_margin_components"] = {
    ...
    def compute_logistics_simplicity(self, product_id):
        """
        ...
        shipping_cost_score = self.compute_shipping_cost_score(product_id)
        size_weight_score = self.compute_size_weight_score(product_id)
        ...
        self.product_score_dict["logistics_simplicity_components"] = {
            ...
            "shipping_cost_score": shipping_cost_score,
            "weight_score": size_weight_score
        }

        print (f"logistics_simplicity_score is : {logistics_simplicity_score}")
        return logistics_simplicity_score

    def compute_supplier_reliability(self, product_id):
        """
    ...
    def compute_tiktok_virality(self, product_id):
        """
    ...
    def compute_compliance_safety(self, product_id):
        """
        Computes the compliance and safety score for a product
        """
        print ("Computing compliance and safety score")
        risk_level = self.compute_risk_level(product_id)

        compliance_safety_score = 1 - risk_level 

        self.product_score_dict["compliance_safety_components"] = {
    ...
    def compute_order_growth(self, product_id):
        ...
        order_this_month = self.products_data.loc[self.products_data["id"] == product_id, 
                                            "orders_this_month"].values[0]
        order_last_month = self.products_data.loc[self.products_data["id"] == product_id, 
    ...
    def compute_search_trend_growth(self, product_id):
        """
        Computes the search trend growth for a product
        """
        print ("Computing search trend growth")
        search_this_month = self.products_data.loc[self.products_data["id"] == product_id, 
                                            "search_this_month"].values[0]
        search_last_month = self.products_data.loc[self.products_data["id"] == product_id, 
    ...
    def compute_review_growth(self, product_id):
        ...
        reviews_this_month = self.products_data.loc[self.products_data["id"] == product_id, 
                                                "reviews_this_month"].values[0]
        reviews_last_month = self.products_data.loc[self.products_data["id"] == product_id, 
    ...
    def compute_seller_density_score(self, product_id):
        """
    ...
    def compute_review_saturation_score(self, product_id):
        """
    ...
    def compute_price_war_score(self, product_id):
        """
        Computes the price war score for a product
        """
        print ("Computing price war score")
        avg_margin = self.products_data.loc[self.products_data["id"] == product_id, 
                                        "avg_margin"].values[0]
        avg_selling_price = self.products_data.loc[self.products_data["id"] == product_id, 
                                            "avg_selling_price"].values[0]
    ...
    def compute_delivery_time_score(self, product_id):
        """
    ...
    def compute_shipping_cost_score(self, product_id):
        """
    ...
    def compute_size_weight_score(self, product_id):
        """
        Computes the size and weight score for a product
        """
        print ("Computing size and weight score")
        size = self.products_data.loc[self.products_data["id"] == product_id, 
                                            "size"].values[0]
        weight = self.products_data.loc[self.products_data["id"] == product_id, 
                                            "weight"].values[0]
    ...
    def compute_fragility_score(self, product_id):
        """
        Computes the fragility score for a product
        """
        print ("Computing fragility score")
        fragility = self.products_data.loc[self.products_data["id"] == product_id, 
                                        "fragility"].values[0]
    ...
    def compute_supplier_rating_score(self, product_id):
        """
    ...
    def compute_moq_score(self, product_id):
        """
    ...
    def compute_supplier_response_rate_score(self, product_id):
        """
    ...
    def compute_order_volume_score(self, product_id):
        """
        Computes the order volume score for a product
        """
        print ("Computing order volume score")
        supplier_completed_orders = self.products_data.loc[self.products_data["id"] == product_id, 
                                        "supplier_completed_orders"].values[0]
        supplier_max_orders = self.products_data.loc[self.products_data["id"] == product_id, 
    ...
    def compute_trend_growth_score(self, product_id):
        ...
        current_tiktok_trend_score = self.products_data.loc[self.products_data["id"] == product_id, 
                                            "current_tiktok_trend_score"].values[0]
        last_month_tiktok_trend_score = self.products_data.loc[self.products_data["id"] == product_id,
    ...
    def compute_creator_adoption_score(self, product_id):
        """
    ...
    def compute_risk_level(self, product_id):
        """
